...
export function debugFont(font: FontDefinition) {
	...
	for (var a = 0; a < font.chars.length; a++) {
		...
		for (var b = 0; b < chr.pixels.length; b += font.shadow ? 4 : 3) {
			buf.setPixel(bx + chr.pixels[b], chr.pixels[b + 1], [
				chr.pixels[b + 2],
				chr.pixels[b + 2],
				chr.pixels[b + 2],
				255,
...
export function unblendBlackBackground(
	img: ImageData,
	r: number,
	g: number,
	b: number
 ...
 */
export function unblendKnownBg(
	img: ImageData,
	bgimg: ImageData,
	shadow: boolean,
	r: number,
 ...
 */
export function decomposeblack(
	rp: number,
	gp: number,
	bp: number,
	r1: number,
...
export function findReadLine(
	...
	if (cols.length > 1) {
		...
		var sorted = getChatColorMono(
			buffer,
			new Rect(x, y - font.basey, w, h),
			cols
		);
	...
	if (chr == null) {
		return { debugArea: { x, y, w, h }, text: "", fragments: [] } as ReturnType<
			typeof readLine
		>;
	}
...
export function getChatColorMono(
	buf: ImageData,
	rect: RectLike,
	colors: ColortTriplet[]
) {
	var colormap = colors.map((c) => ({ col: c, score: 0 }));
	if (
		rect.x < 0 ||
		rect.y < 0 ||
		rect.x + rect.width > buf.width ||
		rect.y + rect.height > buf.height
	) {
		return colormap;
	}
	var data = buf.data;
	...
	for (var colobj of colormap) {
		...
		for (var y = rect.y; y < rect.y + rect.height; y++) {
			for (var x = rect.x; x < rect.x + rect.width; x++) {
				...
				var d =
					Math.abs(data[i] - col[0]) +
					Math.abs(data[i + 1] - col[1]) +
					Math.abs(data[i + 2] - col[2]);
				if (d < maxd) {
					score += maxd - d;
				}
			}
	...
	for (let col of colors) {
		...
		for (var y = rect.y; y < rect.y + rect.height; y++) {
			for (var x = rect.x; x < rect.x + rect.width; x++) {
				...
				var s =
					(((pixel1[0] / 255) * pixel1[1]) / 255) *
					(((pixel2[0] / 255) * (255.0 - pixel2[1])) / 255);
				score += s;
			}
		}
		if (score > bestscore) {
...
export function readLine(
	buffer: ImageData,
	font: FontDefinition,
	colors: ColortTriplet | ColortTriplet[],
	x: number,
	y: number,
	forward: boolean,
	backward = false
	...
	if (typeof colors[0] != "number" && colors.length == 1) {
		colors = colors[0] as ColortTriplet;
	}
	var multicol = typeof colors[0] != "number";
	var allcolors: ColortTriplet[] = multicol
		? (colors as ColortTriplet[])
		: [colors as ColortTriplet];
	...
	var detectcolor = function (sx: number, sy: number, backward: boolean) {
		...
		return getChatColor(
			buffer,
			{ x: sx, y: sy, width: w, height: font.height },
			allcolors
		);
	};
	...
	for (var dirforward of [true, false]) {
		...
		while (true) {
			...
			var chr = col
				? readChar(buffer, font, col, x + dx, y, !dirforward, true)
				: null;
			if (col == null || chr == null) {
				...
				if (
					lastcol &&
					(col[0] != lastcol[0] || col[1] != lastcol[1] || col[2] != lastcol[2])
				) {
					addfrag(dirforward);
				}
				var spaces = "";
 ...
 */
export function readSmallCapsBackwards(
	buffer: ImageData,
	font: FontDefinition,
	cols: ColortTriplet[],
	x: number,
	...
	var sorted =
		cols.length == 1
			? [{ col: cols[0], score: 1 }]
			: getChatColorMono(buffer, new Rect(x, y - font.basey, w, h), cols);
	//loop until we have a match (max 2 cols)
...
export function readChar(
	...
	for (var chr = 0; chr < font.chars.length; chr++) {
		...
		for (var a = 0; a < chrobj.pixels.length; ) {
			var i =
				(chrx + chrobj.pixels[a]) * 4 +
				(y + chrobj.pixels[a + 1]) * buffer.width * 4;
			var penalty = 0;
			if (!shadow) {
				penalty = canblend(
					buffer.data[i],
			...
			if (debugimg) {
				debugimg.setPixel(chrobj.pixels[a], chrobj.pixels[a + 1], [
					penalty,
					penalty,
					penalty,
					255,
		...
		if (debugobj) {
			debugobj.push({
				chr: chrobj.chr,
				score: scores[chr].sizescore,
				rawscore: scores[chr].score,
				img: debugimg!,
			});
		}
	}
	...
	if (debug.printcharscores) {
		scores
			.slice(0, 5)
			.forEach((q) =>
				console.log(q.chr.chr, q.score.toFixed(3), q.sizescore.toFixed(3))
			);
	}
