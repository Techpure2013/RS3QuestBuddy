...
const { webpackImages } = require("alt1/base") as {
  webpackImages: (input: Record<string, any>) => any;
};
var imgs_rs3 = webpackImages({
  chatimg: wrapInPromise(require("./imgs/chatimg.data.png")),
  chatimghover: wrapInPromise(require("./imgs/chatimghover.data.png")),
  chatimgactive: wrapInPromise(require("./imgs/chatimgactive.data.png")),
  continueimg: wrapInPromise(require("./imgs/continueimg.data.png")),
  continueimgdown: wrapInPromise(require("./imgs/continueimgdown.data.png")),
  boxtl: wrapInPromise(require("./imgs/boxtl.data.png")),
  boxtr: wrapInPromise(require("./imgs/boxtr.data.png")),
});

// Helper function to wrap values into promises
function wrapInPromise(value: any) {
  // If value already has `.then()`, return it directly (it's a Promise)
...
let fontmono = require("./fonts/aa_8px_mono.fontmeta.json");
let fontmono2 = require("./imgs/12pt.fontmeta.json");
let fontheavy = require("./fonts/aa_8px_mono_allcaps.fontmeta.json");
...
export default class DialogReader {
  ...
  find(imgref?: ImgRef) {
    ...
    for (let imgs of [imgs_rs3, imgs_leg]) {
      ...
      for (var a in pos) {
        var p = pos[a];
        if (
          imgref.findSubimage(imgs.boxtr, p.x + 492, p.y, 16, 16).length != 0
        ) {
  ...
  readTitle(imgref: ImgRef) {
    ...
    var pos = OCR.findChar(
      ...
      fontheavy,
      [255, 203, 5],
      Math.round(this.pos.width / 2) - 10,
      16,
      20,
      4
    );
    ...
    var read = OCR.readSmallCapsBackwards(
      buf,
      fontheavy,
      [[255, 203, 5]],
      Math.round(this.pos.width / 2) - 10,
  ...
  readOptions(
    imgref: ImgRef | null | undefined,
    locs: ReturnType<DialogReader["findOptions"]>
  ) {
    imgref = this.ensureimg(imgref);
    if (!imgref) {
      return null;
    ...
    var fontcol: OCR.ColortTriplet = this.pos.legacy
      ? [255, 255, 255]
      : [174, 208, 224];
